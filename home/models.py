...
class Order(models.Model):
    ...
    time_from = models.DateTimeField('Дата отправления и время')
    ...
    time_return_transfer = models.DateTimeField('Время обратного трансфера', blank=True, null=True)
    ...
    #     return "Статус: %s" % (self.name)
    def __str__(self, *args, **kwargs):
        return "Дата: %s № %s Имя: %s Дата: %s Статус: %s" % (self.time_from, str(self.id), self.name, str(self.created), self.status)
