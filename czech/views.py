...
from czech.models import  Order, Car, Questions
from czech.forms import CZOrderForm


def orders(request):
    form = CZOrderForm(request.POST)
    ...
    if request.method == "POST" and form.is_valid():
        ...
        subject, from_email, to = 'Ваш заказ Czech Milan holidays', settings.EMAIL_HOST_USER, Order.objects.last().email
        ...
        #     connection=connection
        # )
        return render(request, 'landing_cz/completeOrder.html', locals())
        if not form.is_valid():
            raise forms.ValidationError('Some field is blank')
    return render(request, 'landing_cz/orders.html', locals())
...
def main(request):
    ...
    car_class = Car.objects.all()
    return render(request, 'landing_cz/index.html', locals())


def questions(request):
    questions = Questions.objects.all()
    return render(request, 'landing_cz/questions.html', locals())


def confidential(request):

    return render(request, 'landing_cz/confidential.html', locals())


def oferta(request):

    return render(request, 'landing_cz/oferta.html', locals())
